...
def drawGame(board):
    ...
    for column in range(COLS):
        for row in range(ROWS):
            pygame.draw.rect(WIN, BLUE,(column*UNITSIZE,row*UNITSIZE+UNITSIZE, UNITSIZE, UNITSIZE))
            pygame.draw.circle(WIN, WHITE, (int(column*UNITSIZE+UNITSIZE/2), int(row*UNITSIZE+UNITSIZE+UNITSIZE/2)), RADIUS)
        for column in range(COLS):
            for row in range(ROWS):
                if board[row][column] == PLAYER:
                    pygame.draw.circle(WIN, RED, (int(column*UNITSIZE+UNITSIZE/2), ((ROWS+1)*UNITSIZE)-int(row*UNITSIZE+UNITSIZE/2)), RADIUS)
                if board[row][column] == COMP:
                    pygame.draw.circle(WIN, YELLOW, (int(column*UNITSIZE+UNITSIZE/2), ((ROWS+1)*UNITSIZE)-int(row*UNITSIZE+UNITSIZE/2)), RADIUS)
